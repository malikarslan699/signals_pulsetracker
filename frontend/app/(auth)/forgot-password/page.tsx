      ...
      <div className="mb-6 text-center">
        <div className="flex justify-center mb-4">
          <div className="w-12 h-12 rounded-full bg-long/10 border border-long/20 flex items-center justify-center">
            <Mail className="w-6 h-6 text-long" />
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            className={`w-full px-4 py-2.5 bg-surface-2 border rounded-lg text-sm text-text-primary placeholder-text-muted focus:outline-none focus:border-long transition-colors ${mutation.isError ? "border-short" : "border-border"}`}
          />
        </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-3 bg-long hover:bg-long/90 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-all shadow-lg shadow-long/20"
        >
          {mutation.isPending ? (
            <>
